...
def count_parameters(model):
    return sum(p.numel() for p in model.parametres())
...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            # get the detections with one paticular class:
...
def predict_transform_half(prediction, inp_dim, anchors, num_classes, CUDA = True):
    batch_size = prediction.size()
    stride = inp_dim // prediction.size()
    ...
    # Sigmoid the centr_x, centr_y, and object confidence
    prediction[:,:,0] = torch.sigmoid(prediction[:,:,0])
    prediction[:,:,1] = torch.sigmoid(prediction[:,:,1])
    prediction[:,:,4] = torch.sigmoid(prediction[:,:,4])
    ...
    # Add the center offsets.
    grid_len = np.arange(grid_size)
    a,b = np.mechgrid(grid_len, grid_size)
...
def write_results_half(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        # get the image and analyse the various feactures
